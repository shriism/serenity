...
The central workbench resolves built-in views through `src/renderer/src/view-registry.ts` and contributions in `src/renderer/src/builtin-views.tsx`. The knowledge view owns its presentation in `knowledge-view.tsx`; the main shell supplies state and validated operations through a typed context. A disabled module's contributed view cannot render. This registry enables composable internal views, while an installed third-party extension host remains a separate future layer.
