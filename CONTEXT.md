...
**Commands.** Named built-in actions such as `view.knowledge`, `entity.create`, and `documents.import` are contributed in `src/renderer/src/commands.ts`. Page-opening commands are generated for current workspace pages. The palette, navigation, and page links reuse those identities; the renderer dispatches to existing handlers, and persistent operations still cross validated IPC. The current command API is an **internal contribution list**, not arbitrary user-authored command code or a complete extension API.

**Views and layout.** `src/renderer/src/view-registry.ts` registers built-in central views contributed by `builtin-views.tsx`; the extracted Knowledge renderer lives in `knowledge-view.tsx`. Module-gated views cannot render when their module is disabled; the registry's `available()` check also governs session restoration. The shell still owns substantial view-specific state and handlers. The existing layout is one center canvas, tabbed entities/documents, a compact navigation dock, and a collapsible/expandable contextual AI rail. The center can split into two editor groups (see ARCHITECTURE.md), built on a layout tree that can later grow into tiling; alternative views of the same resource (e.g. entity timeline or graph) are not built yet.
...
### 1. Finish internal resource/view composition

- Move the remaining navigation, active-resource, tab restoration, and view-specific shell state behind coherent internal view/resource contracts. Today some behavior still dispatches by explicit kind or view inside `main.tsx`; a registry entry alone does not make a new resource fully openable.
- Define how an addressable resource chooses a default view, how alternative views can be offered, and what happens when a resource is deleted, a module is disabled, or a page contains invalid YAML. Preserve unsaved-edit prompts and stale-save protection.
- Generalize tabs/panes beyond entity/document tabs and the fixed center/right-rail arrangement, with a workspace-local layout format and safe restoration. Make keyboard focus, active resource, and contextual AI scope predictable when multiple panes are open.
...
The smoke test needs a graphical desktop. `SERENITY_SMOKE_PROVIDER=copilot` or `codex` opts into a live authenticated conversation; `SERENITY_SMOKE_EXECUTABLE` points it at a packaged executable. See [README.md](README.md) for packaging variants and user instructions. Inspect `git status` and the diff before changing or committing files; do not overwrite in-progress work while following this roadmap.

The last completed, pushed foundation commits described in this context are `5f3752c` (workspace pages/resources/commands) and `9fb3d28` (built-in view contributions). Their macOS, Windows and Linux CI jobs passed. This document is a snapshot of the project, not a substitute for reading the current code and tests before editing it.
