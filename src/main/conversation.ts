...
export async function sendMessage(
  ...
  const records = contextRecords(snapshot, files)
  const localMatches = await workspace.search(question)
  const indexedMatches = await rankSemanticIndex(workspace, question)
  const first = prepareContext(records, question, [...indexedMatches, ...localMatches])
  message.sharedContext = [first.shared]
  await workspace.saveConversation(conversation)
