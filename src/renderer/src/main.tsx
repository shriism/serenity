        ...
        : !draft ? <section className="empty"><span className="empty-symbol">◇</span><h1>Start with what you know.</h1><p>Create a person, project, idea, place, or anything else meaningful to you. Categories are yours to define.</p><button className="primary" onClick={newEntity}>Create an entity <span>+</span></button></section>
          : <div className="content">
              ...
              <aside className="details"><span className="eyebrow">CLAIMS & SOURCES</span><h2>What we know</h2><p className="detail-intro">Individual facts stay connected to their source. Conflicting claims remain visible.</p>
                ...
                {claims.map((item) => { const target = workspace.entities.find((entity) => entity.id === item.value); const sourceEntity = workspace.merges.find((merge) => merge.id === item.mergedFrom); return <div className={`claim ${item.status === 'retracted' ? 'retracted' : ''}`} key={item.id}><span className="eyebrow">{item.key} {conflicts.includes(item.key) ? '· CONFLICT' : ''}</span>{target ? <button className="claim-link" onClick={() => selectEntity(target)}>{target.title} ↗</button> : <strong>{item.value}</strong>}<small>From {item.source} · {item.origin === 'human' ? 'direct statement' : item.origin === 'ai-inference' ? 'AI inference' : 'AI extraction'} · {item.status}{item.confidence !== undefined ? ` · AI estimate ${Math.round(item.confidence * 100)}%` : ''}{sourceEntity ? ` · archived from ${sourceEntity.title}` : ''}{item.retractionReason ? ` · ${item.retractionReason}` : ''}</small>{item.status === 'confirmed' && <button className="retract-button" onClick={() => void retractClaim(item.id)}>Retract claim</button>}</div> })}
