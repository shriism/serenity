...
try {
  ...
  assert.equal(editedHome, true, 'The Home page should save editable Markdown and render newly added workspace queries')
  ...
  const linkedEntity = await evaluate(pageUrl, `(async () => { [...document.querySelectorAll('.navigation button')].find((item) => item.textContent?.includes('Home'))?.click(); for (let i = 0; i < 40; i++) { const link = [...document.querySelectorAll('.page-link')].find((item) => item.textContent?.includes('Alex')); if (link) { link.click(); break } await new Promise((resolve) => setTimeout(resolve, 100)) } for (let i = 0; i < 30; i++) { const title = document.querySelector('.editor #title')?.value; if (title === 'Alex') return title; await new Promise((resolve) => setTimeout(resolve, 100)) } return null })()`)
