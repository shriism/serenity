...
test('registered views render through contributions and respect disabled modules', async () => {
  ...
  try {
    ...
    assert.equal(registry.available('calendar', { workspace: await workspace.snapshot() }), true)
    ...
    assert.equal(registry.available('calendar', { workspace: await workspace.snapshot() }), false)
